...
from src.utils.ema import EMA, ema_scope
...
class BaseTrainer:
    ...
    def __init__(
        ...
        logger : Logger or None, optional
            Logger that logs output, by default None.
        """
        self.is_train = True
        ...
        self.sample_step = self.cfg_trainer.get('sample_step', 500)
        ...
        self.train_metric_tracker = MetricTracker(
            ...
            *self.criterion.loss_weight_names,
            'grad_norm',
        ...
        if self.cfg_trainer.get('ema') is not None:
            ema_cfg = self.cfg_trainer.ema
            self.ema = EMA(
                model=self.model,
            ...
            self._log(
                f'EMA enabled with decay={self.ema.decay}, warmup_steps={self.ema.warmup_steps}'
            )
    ...
    def _train_epoch(self, epoch: int) -> dict[str, Any]:
        ...
        for batch_idx, batch in enumerate(pbar):
            try:
                batch = self.process_batch(
                    batch_idx=batch_idx,
                    batch=batch,
                    metric_tracker=self.train_metric_tracker,
                )
            except torch.cuda.OutOfMemoryError as e:
                if self.skip_oom:
                    self._log('OOM on batch. Skipping batch.', message_type='WARNING')
            ...
            if batch_idx > 0 and batch_idx % self.log_step == 0:
                ...
                last_train_metrics = self.train_metric_tracker.result()
                self.train_metric_tracker.reset()
        ...
        # Run val/test (use EMA weights if available)
        for part, dataloader in self.eval_dataloaders.items():
            if self.ema is not None:
                with ema_scope(self.ema, self.model):
    ...
    def _evaluation_epoch(
        ...
        self.is_train = False
        self.model.eval()
        self.eval_metric_tracker.reset()
        ...
        for batch_idx, batch in tqdm(
            ...
            batch = self.process_batch(
                batch_idx=batch_idx,
                batch=batch,
                metric_tracker=self.eval_metric_tracker,
                part=part,
            )

        self.writer.set_step(epoch * self.epoch_len, part)
        self._log_scalars(self.eval_metric_tracker)
        self._log_batch(batch_idx, batch, epoch)
        return self.eval_metric_tracker.result()
    ...
    def _clip_grad_norm(self) -> None:
        ...
        """
        clip_grad_norm(model=self.model, max_grad_norm=self.cfg_trainer.get('max_grad_norm'))
    ...
    def _get_grad_norm(self, norm_type: float | str | None = 2) -> float:
        ...
        float
            The calculated norm.
        """
        total_grad_norm = get_grad_norm(model=self.model, norm_type=norm_type)
    ...
    def _save_checkpoint(self, epoch: int, save_best: bool = False) -> None:
        ...
        if save_best:
            ...
            self._log(f'Saving current best: {filename}...')
        else:
    ...
    def _resume_checkpoint(self, resume_path: Path | str) -> None:
        """
        ...
        """
        model, optimizer, lr_scheduler, start_epoch, mnt_best, ema = resume_checkpoint(
            cfg=self.cfg,
            model=self.model,
            optimizer=self.optimizer,
            lr_scheduler=self.lr_scheduler,
    ...
    def _from_pretrained(self, pretrained_path: Path | str) -> None:
        """
        Init model with weights from pretrained pth file.

        Parameters
        ----------
        pretrained_path : Path or str
