...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        instance_data = self.load_object(data_path)
    ...
    def load_object(self, path: str) -> dict[str, torch.Tensor]:
        """
        ...
        dict[str, torch.Tensor]
            Dict with keys 'u0', 'ut', 't', and 'x'.
        """
        raw = safetensors.torch.load_file(path)
        return raw
    ...
    def _assert_index_is_valid(index: list[dict[str, Any]]) -> None:
        ...
        for entry in index:
            assert 'path' in entry, (
                "Each dataset item should include field 'path' - path to sample file."
            )
